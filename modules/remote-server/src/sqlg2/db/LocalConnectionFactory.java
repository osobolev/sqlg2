...
public final class LocalConnectionFactory implements IConnectionFactory {
     ...
     * Constructor.
     */
    public LocalConnectionFactory(SessionFactory sessionFactory, DBSpecific specific, SQLGLogger logger) {
        this(sessionFactory, specific, logger, false);
    }

    LocalConnectionFactory(SessionFactory sessionFactory, DBSpecific specific, SQLGLogger logger, boolean server) {
    ...
    private DBInterface openConnection(String user, String password, String host, boolean background) throws SQLException {
        SessionFactory.SessionData loginData = sessionFactory.login(globals.getLogger(), user, password);
